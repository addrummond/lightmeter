...
uint8_t read_byte_setting(byte_settings_names_t name, uint8_t value)
{
    uint16_t n = eeprom_read_word(0);

    uint16_t i;
    for (i = 4; i < (n << 1) + 4; i += 2) {
        if (eeprom_read_byte(i) == name) {
            return eeprom_read_byte(i+1);
        }
    }
    return 0;
}
...
{
    uint16_t n = eeprom_read_word(0);
    uint16_t next_block = eeprom_read_word(1);

    uint16_t i;
    for (i = 4; i < (n << 1) + 4; i += 2) {
        if (eeprom_read_byte(i) == name) {
            eeprom_write_byte(i+1, value);
    ...
    eeprom_write_byte(i, name);
    eeprom_write_byte(i+1, value);
    ++n;
    eeprom_write_word(0, n);
    return false;
}
