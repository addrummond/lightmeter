...
uint32_t hamming_get_encoded_message_byte_length_with_init_sequences(uint32_t len, unsigned num_init_sequences);
uint32_t hamming_get_max_output_length_given_input_length(uint32_t len);
void hamming_encode_message(const uint8_t *in, unsigned length, uint8_t *out, unsigned num_init_sequences);
