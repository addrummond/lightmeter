...
static const uint8_t *ssa_get_8px_grid(uint8_t ascii)
{
    uint8_t m;
    if (ascii >= '0' && ascii <= '9') {
        m = '0';
...
diff:
    return CHAR_PIXELS_8PX + CHAR_8PX_0_O + ((ascii - m) << 3);
}
                                      ...
                                      uint8_t x)
{
    ...
    if (x >= func_state->start_x && func_state->i < func_state->len) {
        if (func_state->i < func_state->sso.length) {
            ...
            display_bwrite_12x12_char(ssa_get_8px_grid(ascii), out, pages_per_col, 0);
        }
        else if (func_state->i == func_state->sso.length + 1) {
            display_bwrite_12x12_char(CHAR_8PX_F, out, pages_per_col, 0);
        }
